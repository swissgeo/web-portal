...
export default function zoomToExtent(
  ...
  const options = dispatcherOrNothing
    ? (optionsOrDispatcher as ZoomToExtentOptions)
    : {};
  const dispatcher =
    dispatcherOrNothing ?? (optionsOrDispatcher as ActionDispatcher);
  ...
  if (normalizedExtent) {
    ...
    const centerOfExtent: SingleCoordinate =
      extentUtils.getExtentCenter(normalizedExtent);
    ...
    if (extentSize.height > extentSize.width) {
      targetResolution =
        extentSize.height / (uiStore.height - uiStore.headerHeight);
    } else {
