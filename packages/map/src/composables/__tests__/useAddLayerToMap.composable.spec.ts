...
describe('useAddLayerToMap', () => {
    it('removes the same raw layer instance on unmount', () => {
        ...
        const rawLayer = new Layer({ source: new VectorSource() }) as unknown as BaseLayer
        ...
        const TestComponent = defineComponent({
            setup() {
                ...
                const composable = useAddLayerToMap(olLayer, zIndex, isVisible, opacity, olMap)
    ...
    it('removes previously added layer when a new layer instance is added', () => {
        const fakeMap = createFakeMap()
        const firstRawLayer = new Layer({ source: new VectorSource() }) as unknown as BaseLayer
        const secondRawLayer = new Layer({ source: new VectorSource() }) as unknown as BaseLayer
