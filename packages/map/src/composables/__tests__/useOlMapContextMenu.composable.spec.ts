...
describe('useOlMapContextMenu', () => {
    ...
    describe('right-click (contextmenu)', () => {
        it('opens the popup and sets coordinate on contextmenu event', () => {
            ...
            fakeMap.viewport.dispatchEvent(new MouseEvent('contextmenu', { bubbles: true }))
    ...
    describe('cleanup', () => {
        it('unregisters all event listeners on unmount', () => {
            const removeEventListenerSpy = vi.spyOn(fakeMap.viewport, 'removeEventListener')
            const { wrapper } = mountWithMap(fakeMap)

            wrapper.unmount()

            expect(removeEventListenerSpy).toHaveBeenCalledWith('contextmenu', expect.any(Function))
            expect(fakeMap.un).toHaveBeenCalledWith('singleclick', expect.any(Function))
        })
    })
})
