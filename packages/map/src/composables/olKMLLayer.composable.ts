    ...
    function initialize(): void {
        ...
        features.forEach((feature) => {
            ...
            if (isTextFeature) {
                ...
                if (!feature.get('title') && typeof name === 'string' && name.trim().length > 0) {
            ...
            if (isDrawingKmlLayer || isDrawingFeature(feature)) {
                feature.setStyle(createDrawingFeatureStyleFunction(normalizedFeatureStyle))
                feature.changed()
            } else if (feature.get('isTextFeature')) {
                feature.setStyle(createTextFeatureStyle(String(feature.get('text') ?? '')))
            }
        })
