...
function getOlStyleFromLiterals(
  ...
  const olStyles: OLBasicStyles = vectorOptions
...
function getLabelTemplate(
  value: GeoAdminGeoJSONVectorOptions["label"],
): string | undefined {
  if (value?.template) {
...
class OlStyleForPropertyValue {
  ...
  getOlStyle_(
    ...
    const geomType = getGeomTypeFromGeometry(
    ...
    if (styleSpec && styleSpec.olStyle) {
      ...
      return this.setOlRotation_(
        olStyle,
        styleSpec.imageRotationProperty,
        properties,
      );
    }
    return this.defaultStyle;
  ...
  getFeatureStyle(feature: Feature, resolution: number): Style {
    ...
    if (this.type === "single" && this.singleStyle) {
      ...
      return this.setOlRotation_(
        olStyle,
        this.singleStyle.imageRotationProperty,
        properties,
      );
