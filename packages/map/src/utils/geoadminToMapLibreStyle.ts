...
/**
 * Distinguishes an already-standard MapLibre style (pass straight through to
 * `ol-mapbox-style`) from geoadmin's legacy "literals" style (needs conversion via
 * {@link geoadminToMapLibreStyle}). Legacy styles carry a top-level
 * `type: "single" | "unique" | "range"` and `property`; MapLibre styles carry
 * `version` / `sources` / `layers`. An empty `{}` matches neither.
 */
export function isMapLibreStyle(style: unknown): style is MapLibreStyle {
  return (
    typeof style === "object" &&
    style !== null &&
    typeof (style as MapLibreStyle).version === "number" &&
    Array.isArray((style as MapLibreStyle).layers) &&
    Boolean((style as MapLibreStyle).sources)
  );
}
