...
describe("CoordinateSystemBounds", () => {
  describe("splitIfOutOfBounds(coordinates, bounds)", () => {
    ...
    it("returns a single CoordinatesChunk if no split is needed", () => {
      ...
      expect(singleChunk)
        .to.be.an("Object")
        .that.has.ownProperty("coordinates");
    ...
    it("splits the given coordinates in two chunks if part of it is outside bounds", () => {
      ...
      expect(outOfBoundChunk.coordinates[0]).to.eql(
        coordinatesOverlappingBounds[0],
      );
    ...
    it("handles properly a line going multiple times out of bounds", () => {
      ...
      const expectedThirdIntersection: SingleCoordinate = [
        bounds.upperX,
        bounds.upperY,
      ];
      ...
      expect(secondChunk.coordinates[1]).to.eql(
        coordinatesGoingBackAndForth[1],
      );
      ...
      expect(fourthChunk.coordinates[1]).to.eql(
        coordinatesGoingBackAndForth[4],
      );
      expect(fourthChunk.coordinates[2]).to.eql(
        coordinatesGoingBackAndForth[5],
      );
    });
    ...
    it('handles some "real" use case well', () => {
      ...
      const reversedResult = LV95.bounds.splitIfOutOfBounds(
        sample1.toReversed(),
      );
      expect(reversedResult).to.be.an("Array").of.length(3);
      const [firstReversedChunk, secondReversedChunk, thirdReversedChunk] =
        reversedResult;
      ...
      expect(firstReversedChunk.coordinates[1][0]).to.approximately(
        2855830.1,
        0.1,
      );
      expect(firstReversedChunk.coordinates[1][1]).to.approximately(
        1030000,
        0.1,
      );
      ...
      expect(secondReversedChunk.coordinates[0][0]).to.approximately(
        2855830.1,
        0.1,
      );
      expect(secondReversedChunk.coordinates[0][1]).to.approximately(
        1030000,
        0.1,
      );
      expect(secondReversedChunk.coordinates[1][0]).to.approximately(
        2674764.8,
        0.1,
      );
      expect(secondReversedChunk.coordinates[1][1]).to.approximately(
        1350000,
        0.1,
      );
      ...
      expect(thirdReversedChunk.coordinates[0][0]).to.approximately(
        2674764.8,
        0.1,
      );
      expect(thirdReversedChunk.coordinates[0][1]).to.approximately(
        1350000,
        0.1,
      );
