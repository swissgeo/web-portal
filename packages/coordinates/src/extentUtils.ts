...
export function projExtent<T extends FlatExtent | NormalizedExtent>(
  ...
  if (extent.length === 4) {
    ...
    const bottomLeft = proj4(fromProj.epsg, toProj.epsg, extent[0]).map(
      (value) => toProj.roundCoordinateValue(value),
    );
...
export function getExtentIntersectionWithCurrentProjection(
  ...
  let intersectionOfExtents = bbox(
    bboxClip(
      bboxPolygon(currentProjectionAsExtentProjection),
...
export function getExtentCenter(
  extent: FlatExtent | NormalizedExtent,
): SingleCoordinate {
...
export function createPixelExtentAround(
  ...
  const extent: FlatExtent = projExtent(
    WGS84,
    projection,
    bbox(bufferAround) as FlatExtent,
  );
