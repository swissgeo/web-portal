...
describe("Unit test for coordinatesUtils", () => {
  describe("toRoundedString", () => {
    ...
    it("rounds with decimal if a number is given as digits", () => {
      expect(coordinatesUtils.toRoundedString([1.44, 2.44], 1)).to.eq(
        "1.4, 2.4",
      );
      expect(coordinatesUtils.toRoundedString([1.45, 2.45], 1)).to.eq(
        "1.5, 2.5",
      );
    });
    it("correctly enforcers digits when asked for", () => {
      expect(
        coordinatesUtils.toRoundedString([1.44, 2.44], 5, false, true),
      ).to.eq("1.44000, 2.44000");
      expect(coordinatesUtils.toRoundedString([1, 2], 3, false, true)).to.eq(
        "1.000, 2.000",
      );
      expect(
        coordinatesUtils.toRoundedString([1234.5678, 1234.5678], 6, true, true),
      ).to.eq("1'234.567800, 1'234.567800");
    });
  });
  ...
  describe("wrapXCoordinates()", () => {
    ...
    it("do not wrap if projection is not global (world-wide)", () => {
      ...
      expect(
        coordinatesUtils.wrapXCoordinates(justOffBoundCoordinate, LV95),
      ).to.deep.equal(justOffBoundCoordinate);
    });
    it("can wrap every coordinates of an array of coordinates", () => {
      function testMultipleWrap(projection: CoordinateSystem) {
        ...
        const inBoundCoordinate: SingleCoordinate = [
          bounds.lowerX,
          bounds.center[1],
        ];
        const inBoundCoordinate2: SingleCoordinate = [
          bounds.center[0],
          bounds.center[1],
        ];
        const inBoundCoordinate3: SingleCoordinate = [
          bounds.upperX,
          bounds.center[1],
        ];
        ...
        expect(second).to.deep.equal(
          inBoundCoordinate,
          "wrong lowerX handling",
        );
        expect(third).to.deep.equal(
          inBoundCoordinate2,
          "wrong center handling",
        );
        expect(fourth).to.deep.equal(
          inBoundCoordinate3,
          "wrong upperX handling",
        );
  ...
  describe("unwrapGeometryCoordinates(coordinates)", () => {
    it("returns the input if nothing is required", () => {
      expect(coordinatesUtils.unwrapGeometryCoordinates([]))
        .to.be.an("Array")
        .lengthOf(0);
      ...
      expect(
        coordinatesUtils.unwrapGeometryCoordinates(alreadyUnwrappedCoordinates),
      ).to.eql(alreadyUnwrappedCoordinates);
    });
    it("unwraps when required", () => {
      ...
      expect(
        coordinatesUtils.unwrapGeometryCoordinates(wrappedCoordinates),
      ).to.eql(expectedOutcome);
    });
