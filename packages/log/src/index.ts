  ...
  return messages.flatMap((message) => {
    if (
      ...
      if ("title" in message && typeof message.title === "string") {
        ...
        let titleBackgroundColor: LogPreDefinedColor | string =
          LogPreDefinedColor.Slate;
...
const log: SwissGeoLog = {
  ...
  error: (...messages: SwissGeoLogInput[]) =>
    logToConsole(LogLevel.Error, messages),
  warn: (...messages: SwissGeoLogInput[]) =>
    logToConsole(LogLevel.Warn, messages),
  info: (...messages: SwissGeoLogInput[]) =>
    logToConsole(LogLevel.Info, messages),
  debug: (...messages: SwissGeoLogInput[]) =>
    logToConsole(LogLevel.Debug, messages),
};
export default log;
