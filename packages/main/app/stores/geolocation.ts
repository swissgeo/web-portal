...
function handleNewGeolocationPosition(
  ...
  const positionProjected: SingleCoordinate = proj4(
    WGS84.epsg,
...
function setCenterIfInBounds(
  ...
  const lv95BoundsInCurrentProjection = LV95.getBoundsAs(
  ...
  if (lv95BoundsInCurrentProjection?.isInBounds(center)) {
    if (!isEqual(positionStore.center as number[], center as number[])) {
      ...
      if (store.firstTime) {
        ...
        positionStore.setZoom(
          positionStore.projection.get1_25000ZoomLevel(),
          dispatcher,
        );
      }
    }
    ...
    log.warn({
      ...
      titleColor: LogPreDefinedColor.Amber,
      messages: [
        `Current geolocation is out of bounds: ${JSON.stringify(center)}`,
      ],
    });
    toaster.showWarning(
