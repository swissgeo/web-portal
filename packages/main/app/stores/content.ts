...
export const useContentStore = defineStore("contentStore", () => {
  ...
  /**
   * Since we have nested routes, we look for the first occurrence of the other
   * language's document ID in the route tree
   */
  function getPageRouteInLanguage(lang: Language): RouteRecordRaw | null {
    const documentId = currentPageData.value?.languageReferences?.find(
      (reference: LanguageReference) =>
        reference.metadata.language.locale === lang,
