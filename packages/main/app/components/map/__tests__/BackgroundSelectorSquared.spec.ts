...
type SquaredVM = { selectBackgroundCallback: (_layer: Layer | VoidLayer) => void }
...
describe('BackgroundSelectorSquared.vue', () => {
    it('always renders the trigger entry', () => {
        ...
        expect(wrapper.findAll('map-background-selector-entry-stub').length).toBeGreaterThanOrEqual(
            1
        )
    })
    ...
    it('shows no option entries when the selector is closed', () => {
        ...
        expect(wrapper.findAll('map-background-selector-entry-stub').length).toBe(1)
    })
