...
const selectedFeature = computed<Feature<Geometry> | null>(() => {
  ...
  return (
    features.find((feature) => resolveFeatureId(feature) === selectedId) ?? null
  );
});
...
const isPointFeatureSelected = computed(
  () => selectedFeatureKind.value === "Point",
);
const isTextFeatureSelected = computed(
  () => selectedFeatureKind.value === "Text",
);
const isLineFeatureSelected = computed(
  () => selectedFeatureKind.value === "LineString",
);
const isPolygonFeatureSelected = computed(
  () => selectedFeatureKind.value === "Polygon",
);
...
function deleteSelectedFeature() {
  ...
  const features =
    drawingStore.drawingFeatures as unknown as Feature<Geometry>[];
...
function getFeatureMetric(metric: "area" | "perimeter"): number | undefined {
  const info = drawingStore.selectedFeatureInfo as Record<
    string,
    unknown
  > | null;
  if (!info) {
...
<template>
  ...
  >
    <UCard>
      ...
      <div v-if="drawingStore.selectedFeatureInfo" class="space-y-2 text-sm">
        <p>
          <span class="font-medium"
            >{{ t("debug.drawingFeatureInfo.typeLabel") }}:</span
          >
        ...
        <template v-if="isDescriptionEditableFeatureSelected">
          ...
          <div>
            <div class="mb-1 flex items-center justify-between">
              ...
              <div class="flex items-center gap-1">
                ...
                >
                  <Eye v-if="editorState.isDescriptionVisible" :size="14" />
          ...
          >
            ...
            <div>
              ...
              <div class="grid grid-cols-6 gap-1">
                ...
                >
                  <img :src="icon.dataUrl" :alt="icon.name" class="h-4" />
                </button>
              </div>
            </div>
          </div>
          ...
          <div
            v-if="
              showTextStyleMenu &&
              (isPointFeatureSelected || isTextFeatureSelected)
            "
            class="space-y-2 rounded border border-gray-300 bg-gray-50 p-2"
          >
            <div>
        ...
        <p
          v-if="
            ['Point', 'Text'].includes(drawingStore.selectedFeatureInfo.kind)
          "
        >
          <span class="font-medium"
