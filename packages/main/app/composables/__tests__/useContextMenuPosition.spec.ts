...
const { w3wData, elevationData, w3wPending, elevationPending } = await vi.hoisted(async () => {
...
async function setup(coordinate: [number, number] | null = BERN_LV95) {
    const { useContextMenuPosition } = await import('~/composables/useContextMenuPosition')
...
describe('useContextMenuPosition', () => {
    ...
    it('formats elevation correctly in meters and feet', async () => {
        ...
        const { rows } = await setup()
        const elevRow = rows.value.find((r) => r.label === 'map.contextMenuPopup.elevation')
