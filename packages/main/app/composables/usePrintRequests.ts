...
import type { PrintConfig, PrintFormat, PrintOrientation } from '../types/print'
 ...
 * }
 */
// export type PrintPostRequestBody = PrintConfig & {
//     stateId: string
// }
...
function convertToValidPrintPostRequestBody(
    stateId: string,
    ...
    return {
        ...
        print_orientation: printConfig.orientation,
        print_resolution: printConfig.resolution,
    }
}
...
const usePrintRequestsStore = defineStore('printRequests', () => {
    ...
    if (import.meta.client) {
        ...
        watch(
            requestCollection,
            (newCollection) => {
                localStorage.setItem(PRINT_REQUESTS_STORAGE_KEY, JSON.stringify(newCollection))
            },
            { deep: true }
        )
    }
...
export function usePrintRequests() {
    ...
    const ongoingRequests = computed(() => {
        return requestCollection.value.filter((request) => request.lastResponse.status === 'open')
    })
    ...
    const errorRequests = computed(() => {
        return requestCollection.value.filter((request) => request.lastResponse.status === 'error')
    })
     ...
     */
    async function sendPrintRequest(stateId: string, printConfig: PrintConfig) {
        const body = convertToValidPrintPostRequestBody(stateId, printConfig)
