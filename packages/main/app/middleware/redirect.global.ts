...
export function resolvePreferredLocale(): string {
    ...
    let acceptLanguage: string | undefined = useRequestHeaders(['accept-language'])[
        'accept-language'
    ]
...
export function redirector(to: Pick<RouteLocationNormalized, 'path' | 'query'>) {
    ...
    if (isExempt(to.path)) {
        ...
        return navigateTo({ path: localePath('/map'), query: to.query }, { redirectCode: 302 })
    } else if (isRootPath(to.path)) {
        const locale = resolvePreferredLocale()
        return navigateTo({ path: `/${locale}/map`, query: to.query }, { redirectCode: 302 })
    } else if (!isLocalized(to.path)) {
        // it's not localized, but is a specific path. We prepend the locale instead of redirecting to map
        const locale = resolvePreferredLocale()
        return navigateTo({ path: `/${locale}${to.path}`, query: to.query }, { redirectCode: 302 })
    }
}
