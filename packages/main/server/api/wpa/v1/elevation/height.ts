...
export default defineEventHandler(async (event) => {
  ...
  if (isNaN(latNum) || isNaN(lonNum)) {
    throw createError({
      statusCode: 400,
      statusMessage: "lat and lon must be numeric",
    });
  }
  ...
  try {
    ...
    const data = await $fetch<{ height: string }>(
      `${config.geoadminApiBaseUrl}/height`,
      {
    ...
    log.error(
      `Elevation upstream error for lat=${latNum}, lon=${lonNum}: ${String(error)}`,
    );
    throw createError({
      statusCode: 500,
