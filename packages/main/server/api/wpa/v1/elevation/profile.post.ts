...
function isValidLineString(
  body: unknown,
): body is { geojson: LineString; sr?: number } {
...
function reprojectCoordinatesToLV95(
...
async function fetchProfileChunk(
  coordinates: Position[],
): Promise<RawProfilePoint[]> {
  const config = useRuntimeConfig();
  return $fetch<RawProfilePoint[]>(
    `${config.geoadminApiBaseUrl}/profile.json`,
    {
      method: "POST",
...
function computeMetadata(
  ...
  const elevationDifference =
    (points.at(-1)?.elevation ?? 0) - (points[0]?.elevation ?? 0);
...
export default defineEventHandler(async (event) => {
  ...
  const lv95Coordinates = reprojectCoordinatesToLV95(
