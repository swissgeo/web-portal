...
<template>
    ...
    <li ref="item"
        ...
        :data-cy="`search-result-entry-${entry.resultType.toLowerCase()}`" :tabindex="index === 0 ? 0 : -1"
        @keydown.up.prevent="goToPrevious" @keydown.down.prevent="goToNext" @keydown.home.prevent="goToFirst"
        @keydown.end.prevent="goToLast" @keyup.enter="selectItem" @click="selectItem">
        ...
        <div class="min-w-0 flex-1">
            ...
            <div v-if="entry.description" class="mt-1 truncate text-sm text-surface-600">
                {{ entry.description }}
            </div>
        </div>
    </li>
