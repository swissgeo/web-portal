...
function extractGeoJsonDataUrl(distribution: Distribution | null): string | null {
...
function extractGeoJsonStyleUrl(distribution: Distribution | null): string | null {
    if (!distribution || !distribution.links) {
        return null
    }

    const datasetLinks = distribution.links.filter((link) => link.rel === 'styledby')
