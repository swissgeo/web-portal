...
vi.mock("@swissgeo/shared", () => ({
  ...
  formatDistanceKilometers: vi.fn(
    (meters: number) => `${(meters / 1000).toFixed(2)} km`,
  ),
...
function mountWithMap(
  ...
  return {
    ...
    layerRef,
    zIndexRef,
    api: (wrapper.vm as unknown as { api: ReturnType<typeof useOlDrawing> })
      .api,
  };
}
...
describe("useOlDrawing", () => {
  beforeEach(() => {
    ...
    drawingStoreMock.setDrawingFeatures.mockImplementation(
  ...
  describe("startDrawing (triggered via drawingMode store state)", () => {
    ...
    it("starts measurement drawing and resolves path subtype on drawend", async () => {
      ...
      expect(drawingStoreMock.setMeasurementSubtype).toHaveBeenCalledWith(
        "Path",
      );
      expect(drawingStoreMock.updateFeatureAttributes).toHaveBeenCalledWith(
        measurementFeature,
  ...
  describe("restoreFeatures (called on mount)", () => {
    it("restores features from drawingFeatures store on mount", () => {
      ...
      expect(drawingStoreMock.ensureFeatureAttributes).toHaveBeenCalledWith(
        feature,
      );
    });
    ...
    it("sets drawing name from layer info on mount", () => {
      ...
      expect(drawingStoreMock.setDrawingName).toHaveBeenCalledWith(
        "My Drawing",
      );
    });
  });
