...
export function useOlDrawing(
  ...
  const resolveColoredMarkerSource = (icon: MarkerIcon, color?: string) => {
    ...
    try {
      ...
      const recoloredSvg = decodedSvg
        .replace(
          /fill="#(?!fff\b|ffffff\b)[0-9a-fA-F]{3,6}"/g,
          `fill="${color}"`,
        )
        .replace(
          /stroke="#(?!fff\b|ffffff\b)[0-9a-fA-F]{3,6}"/g,
          `stroke="${color}"`,
        );
  ...
  const styleFunction = (feature: Feature<Geometry>): StyleLike => {
    ...
    const textColor =
      typeof styleProps.textColor === "string" &&
      styleProps.textColor.length > 0
        ? styleProps.textColor
        : "#111827";
    const textSize =
      typeof styleProps.textSize === "number" &&
      Number.isFinite(styleProps.textSize)
    ...
    if (geomType === "Point") {
      ...
      const iconSize =
        ...
        Number.isFinite(styleProps.iconSize)
          ...
          measurementStyles.push(
            new Style({
              ...
              text: toMeasurementLabelStyle(
                formatDistanceKilometers(totalLength),
              ),
            }),
          );
        }
    ...
    const fillColor =
      ...
      styleProps.fillColor.length > 0
        ? styleProps.fillColor
        : "#ff0000";
    const fillOpacity =
      typeof styleProps.fillOpacity === "number" &&
      Number.isFinite(styleProps.fillOpacity)
          ...
          geometry: (styledFeature) => {
            ...
            if (styledGeometry.getType() === "Polygon") {
              ...
              return ringVertices.length > 0
                ? new MultiPoint(
                    ringVertices.map((coordinate) => [...coordinate]),
                  )
                : undefined;
            }
  ...
  let activeViewportDblClickListener: ((event: MouseEvent) => void) | null =
    null;
  ...
  function addDrawingInteraction(
    ...
    const getFeatureCoordinatesWithoutExtraPoint = (
    ...
    if (type === "Measurement") {
      currentDraw.on("drawstart", (event) => {
        ...
        measurementGeometryListener = geometry.on("change", () => {
          const coordinates = geometry.getCoordinates();
          drawingStore.setMeasurementSubtype(
            coordinates.length >= 3 ? "Path" : "Radius",
          );
        });
      });
    }
    ...
    if (isLineFirstDrawMode) {
      currentDraw.on("drawstart", (_event) => {
        ...
        lineSketchAddFeatureListener = sketchSource.on(
          "addfeature",
          (addFeatureEvent) => {
            checkIfSnappingOnFirstPoint(
              addFeatureEvent.feature as Feature<Geometry>,
            );
          },
        );
      });
    ...
    currentDraw.on("drawend", (event) => {
      ...
      if (isLineFirstDrawMode) {
        const geometry = event.feature.getGeometry();
        if (geometry instanceof Polygon) {
          const lineCoordinates = getFeatureCoordinatesWithoutExtraPoint(
            event.feature,
          );
  ...
  function deleteVertexAtCoordinate(
    feature: Feature<Geometry>,
    coordinate: number[],
  ): boolean {
    const geometry = feature.getGeometry();
    ...
    if (geometry.getType() === "LineString") {
      ...
      const nextCoordinates = coordinates.filter(
    ...
    if (geometry.getType() === "Polygon") {
      ...
      const nextUniqueVertices = uniqueVertices.filter(
        (_, index) => index !== vertexIndex,
      );
  ...
  function insertVertexAtCoordinate(
    feature: Feature<Geometry>,
    coordinate: number[],
  ): boolean {
    const geometry = feature.getGeometry();
  ...
  function endpointHandleNearCoordinate(
    ...
    const closestHandle =
      endpointHandleSource.getClosestFeatureToCoordinate(coordinate);
    if (!closestHandle) {
      return null;
    }
  ...
  function resolveOffsetEndpointHandleCoordinate(
    ...
    return [
      endpointX + normalizedX * offsetDistance,
      endpointY + normalizedY * offsetDistance,
    ];
  }
  ...
  function syncMeasurementSubtypeFromGeometry(feature: Feature<Geometry>) {
    ...
    const featureKind = resolveFeatureKind(feature);
    if (
      featureKind !== "MeasurementRadius" &&
      featureKind !== "MeasurementPath"
    ) {
      return;
    }
    ...
    drawingStore.updateFeatureAttributes(feature, {
      kind:
        measurementSubtype === "Path" ? "MeasurementPath" : "MeasurementRadius",
      measurementSubtype,
      style: {
        intervalKilometers:
          measurementSubtype === "Path"
  ...
  function restoreActiveEditingAfterEndpointExtension(
    ...
    if (reverseExtendedFeatureOnFinish) {
      ...
      if (geometry && geometry.getType() === "LineString") {
        ...
        lineGeometry.setCoordinates(
          [...lineGeometry.getCoordinates()].reverse(),
        );
      }
  ...
  function startEndpointExtension(
    targetFeature: Feature<Geometry>,
    appendAtStart: boolean,
  ) {
  ...
  function enableActiveEditing() {
    ...
    activeClickListener = olMap!.value!.on("singleclick", (event) => {
      ...
      if (handleFeature) {
        const targetFeatureId = String(
          handleFeature.get("__drawingFeatureId") ?? "",
        );
    ...
    activeViewportDblClickListener = (event: MouseEvent) => {
      ...
      if (
        !endpointExtensionInProgress &&
        Date.now() > suppressDoubleClickZoomUntil
      ) {
        return;
      }
  ...
  function applySelectionHighlight(selectedFeature: Feature<Geometry> | null) {
    const selectedFeatureId = selectedFeature
      ? resolveFeatureId(selectedFeature)
      : null;

    source.getFeatures().forEach((feature) => {
      const isSelected =
        selectedFeatureId !== null &&
        resolveFeatureId(feature) === selectedFeatureId;
  ...
  function enablePassiveInspection(
    ...
    passiveMoveListener = olMap!.value!.on("pointermove", (event) => {
      ...
      const hoveredFeature = featureAtPixel(event.pixel);
      olMap!.value!.getViewport().style.cursor = hoveredFeature
        ? "pointer"
        : "";
  ...
  function disablePassiveInspection() {
    ...
    if (passiveOutListener) {
      olMap!
        .value!.getViewport()
        .removeEventListener("pointerleave", passiveOutListener);
  ...
  function restoreFeatures() {
    ...
    if (layer.value.data && typeof layer.value.data === "string") {
      try {
        const metadataDrawingName = drawingStore.extractDrawingNameFromKML(
          layer.value.data,
        );
