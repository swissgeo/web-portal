...
import type { Geometry, LinearRing } from "ol/geom";
...
export function resolveFeatureId(feature: Feature<Geometry>): string {
  ...
  const runtimeFeatureId = String(
    feature.get("__drawingFeatureId") ?? getUid(feature),
  );
  feature.set("__drawingFeatureId", runtimeFeatureId, true);
  return runtimeFeatureId;
}

export function withKmlDocumentMetadata(
  kmlString: string,
  drawingName: string,
): string {
  if (
  ...
  for (const childNode of Array.from(documentElement.childNodes)) {
    ...
    const childElement = childNode as Element;
    if (
      childElement.localName === "name" ||
      childElement.localName === "description"
    ) {
  ...
  if (
    xmlDeclaration.length > 0 &&
    !serialized.trimStart().startsWith("<?xml")
  ) {
...
export function withGpxMetadata(
  gpxString: string,
  drawingName: string,
): string {
...
export function extractDrawingNameFromKML(kmlString: string): string | null {
  const documentMatch = kmlString.match(
    /<Document[\s\S]*?<name>([\s\S]*?)<\/name>/,
  );
...
function createGeometryStyle(
  ...
  const strokeColor =
    typeof styleRecord.strokeColor === "string" &&
    styleRecord.strokeColor.length > 0
      ? styleRecord.strokeColor
      : "#ff0000";

  const strokeWidth =
    typeof styleRecord.strokeWidth === "number" &&
    Number.isFinite(styleRecord.strokeWidth)
      ? Math.max(1, styleRecord.strokeWidth)
      : 2;

  const strokeOpacity =
    typeof styleRecord.strokeOpacity === "number" &&
    Number.isFinite(styleRecord.strokeOpacity)
  ...
  const fillColor =
    typeof styleRecord.fillColor === "string" &&
    styleRecord.fillColor.length > 0
      ? styleRecord.fillColor
      : "#ff0000";

  const fillOpacity =
    typeof styleRecord.fillOpacity === "number" &&
    Number.isFinite(styleRecord.fillOpacity)
...
export function resolveFeatureKind(
  ...
  if (context.drawingMode === "Measurement") {
    return context.measurementSubtype === "Radius"
      ? "MeasurementRadius"
      : "MeasurementPath";
  }
...
export function resolveFeatureStyle(
  ...
  STYLE_PROPERTY_KEYS.forEach((propertyKey) => {
    ...
    if (propertyValue !== undefined && propertyValue !== null) {
      (mergedStyle as Record<StylePropertyKey, unknown>)[propertyKey] =
        propertyValue;
    }
  });
...
export function ensureFeatureAttributes(
  ...
  const title =
    typeof feature.get("title") === "string"
      ? String(feature.get("title"))
      : "";
  const description =
    typeof feature.get("description") === "string"
...
export async function exportFeaturesToKMZBlob(
  ...
  MARKER_ICONS.forEach((icon) => {
    if (usedIconIds.has(icon.id)) {
      modifiedKML = modifiedKML
        .split(icon.dataUrl)
        .join(`icons/${icon.id}.svg`);
    }
  });
...
export function exportFeaturesToGPXBlob(
  features: Feature<Geometry>[],
  drawingName: string,
): Blob {
