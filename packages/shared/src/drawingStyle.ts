...
export function resolveColoredSvgDataUrl(
  source: string,
  color?: string,
): string {
  ...
  try {
    ...
    const recoloredSvg = decodedSvg
      .replace(
        /fill="#(?!fff\b|ffffff\b)[0-9a-fA-F]{3,6}"/gi,
        `fill="${color}"`,
      )
      .replace(
        /stroke="#(?!fff\b|ffffff\b)[0-9a-fA-F]{3,6}"/gi,
        `stroke="${color}"`,
      );
...
export function isDrawingFeature(feature: FeatureLike): boolean {
  return Boolean(
    feature.get("kind") ||
    feature.get("drawingStyle") ||
    feature.get("isTextFeature"),
  );
}
...
export function resolveDrawingFeatureKind(
  ...
  if (geometryType === "LineString") {
    const intervalFromStyle = parseNumeric(
      resolvedStyleProps.intervalKilometers,
    );
    const intervalFromFeature = parseNumeric(feature.get("intervalKilometers"));
    const hasMeasurementInterval =
      typeof intervalFromStyle === "number" ||
      typeof intervalFromFeature === "number";
  ...
  const hasIcon =
    (typeof resolvedStyleProps.iconId === "string" &&
      resolvedStyleProps.iconId.length > 0) ||
...
function resolveLineStringCoordinates(feature: FeatureLike): number[][] {
  ...
  const flatCoordinates =
    typeof maybeGeometry.getFlatCoordinates === "function"
      ...
      : undefined;
  const stride =
    typeof maybeGeometry.getStride === "function"
      ? (maybeGeometry.getStride() ?? 2)
      : 2;
  ...
  for (
    let index = 0;
    index + (stride - 1) < flatCoordinates.length;
    index += stride
  ) {
...
export function createDrawingFeatureStyleFunction(
  ...
  return (feature: FeatureLike, resolution: number): Style | Style[] | void => {
    ...
    const isDescriptionVisible = parseBoolean(
      feature.get("isDescriptionVisible"),
    );
    ...
    if (kind === "Point" && geometryType === "Point") {
      const pointTitle = String(
        feature.get("title") ?? feature.get("name") ?? "",
      ).trim();
      const iconSize =
