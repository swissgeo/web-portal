...
export function isInstanceOfLayerStateConfig(
  object: unknown,
): object is LayerStateConfig {
  ...
  Object.keys(maybeLayerState).forEach((key) => {
    if (!layerStateConfigKeys.includes(key)) {
      throw new Error(
        `The following key ${key} is not allowed in layers state configuration`,
      );
    }
  });
  ...
  if (!("layerUrl" in maybeLayerState)) {
    throw new Error(
      "mandatory attribute layerUrl not present in layer state configuration",
    );
  }

  if (!("type" in maybeLayerState)) {
    throw new Error(
      "mandatory attribute type not present in layer state configuration",
    );
  }
  if (!("isVisible" in maybeLayerState)) {
    throw new Error(
      "mandatory attribute isVisible not present in layer state configuration",
    );
  }
  if (!("opacity" in maybeLayerState)) {
    throw new Error(
      "mandatory attribute opacity not present in layer state configuration",
    );
  }
 ...
 */
export function isInstanceOfAppStateConfig(
  object: unknown,
): object is AppStateConfig {
  ...
  Object.keys(maybeAppState).forEach((key) => {
    if (!validAppStateConfigKeys.includes(key)) {
      throw new Error(
        `The following key is not a valid app State key : ${key}`,
      );
    }
  });
  ...
  if (
    ...
    maybeAppState.map.center.filter(
  ...
  if (
    maybeAppState.backgroundLayer !== undefined &&
    maybeAppState.backgroundLayer !== null
  ) {
...
function isInstanceOfAppStatePayload(
  object: unknown,
): object is AppStatePayload {
...
function validateMap(
  map: AppStateConfig["map"],
): asserts map is AppStateConfig["map"] {
  if (
    ...
    throw new Error(
      "map.center must be a [x, y] number array within LV95 bounds",
    );
  }
...
function validateLayerConfig(
  layer: LayerStateConfig,
): asserts layer is LayerStateConfig {
  if (layer.opacity < 0 || layer.opacity > 1) {
    throw new Error(
      `${layer.layerUrl} opacity must be a number between 0 and 1`,
    );
  }
}
