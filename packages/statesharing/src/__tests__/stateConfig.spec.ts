...
const validState: AppStateConfig = {
  ...
  layers: [
    {
      layerUrl:
        "https://api.example.com/ogc/items/ch.swisstopo.pixelkarte-farbe",
      type: "wmts",
      isVisible: true,
      opacity: 1,
    },
...
describe("validateAndPrepareAppStatePayload", () => {
  ...
  it("rejects null input", () => {
    expect(() => validateAndPrepareAppStatePayload(null)).toThrow(
  ...
  it("rejects unsupported version", () => {
    expect(() =>
      ...
      `The version 2 does not satisfy the constraint ${APP_STATE_CONFIG_CONSTRAINT}`,
    );
  });
